...
class Reta: 
    ...
    def CoefLinear(self,m): #m é o coeficiente angular q vem de inclinação, para fazer a AGREGAÇÃO
        self.m = m
        # print(f"y: {self.coo[0][1]} e x: {self.coo[0][0]}")
...
class TriangEquilatero: #triangulo Triangulo_Equilatero #A,B,C são os lados iguais
    ...
    def perimetro(self):
        if self.a == self.b and self.b == self.c:
            P = 3*self.a #é a soma dos lados, só q como a=b=c, pode ser feito a*3, ao inves de a+b+c.
            return P
        else:
            print("as entradas não são válidas aqui.")
...
class TriangEscaleno(TriangEquilatero): #herança, segundo filho
    def __init__(self,a,b,c):
        self.a = a
        self.b = b
        self.c = c

    def menorAngulo(self): #encapsualmento
        m = min(self.a,self.b,self.c) #achar angulo, para assim conseguir achar a aluta, e com altura achar a area. h= c*sen(teta), a = h*b/2
        aux = 180/(self.a + self.b + self.c)
        menorTeta = aux*m
        return menorTeta
    
    def maiorAngulo(self): #encapsulamento
...
class Retangulo(Quadrado): #herança
    def __init__(self, a,b):
        self.a = a
        self.b = b
        super().__init__(a) #declaração da classe Quadrado, o herança multipla , o numero de variaveis da mae tem q ser menor que o do filho
...
class Poligono(ABC): #class abstrata que será modelo referência para os poligonos: pentagono, hexagono, trapezio e etc.
    def __init__(self,a):
        self.a = a
    @abstractmethod
    def area(self):
...
class TrapezioIsosceles(Poligono):
...
class TrapezioRetangulo(Poligono):
    def __init__(self,b,B,h):
        self.b = b
        self.B = B
        self.h = h
